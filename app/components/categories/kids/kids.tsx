...
function Kids() {
  ...
  const categoryProducts = useAppSelector(
    (state) => state.productSlice.categoryProducts["Kids Items"] ?? []
  );
  const status = useAppSelector((store) => store.productSlice.status);

  useEffect(() => {
    dispatch(fetchProductsByCategory("Kids Items"));
  }, [dispatch]);
  ...
  return (
    <div className="w-[98%] mx-auto px-4 sm:px-6 lg:px-8 mt-7">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl font-semibold text-left">Kids Items</h1>
