...
//   IOrderData,
//   IOrderProduct,
// } from "@/lib/store/user/order/order-slice-type";
...
//     const orderProducts: IOrderProduct[] = selectedItems.map((item) => ({
...
export default function CheckoutPage() {
  ...
  // ✅ Only use selected cart items
  const selectedItems = items.filter(
    (item) => (item.id && selectedItemIds.includes(item.id)) || selectedItemIds.includes(item.productId || "")
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // ✅ Prepare products array with valid productId
